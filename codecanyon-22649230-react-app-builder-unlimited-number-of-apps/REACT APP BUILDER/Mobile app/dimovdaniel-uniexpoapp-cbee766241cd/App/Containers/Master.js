...
import {Button,Text,View,FlatList,TouchableOpacity,StyleSheet,Image,ActivityIndicator,ImageBackground,Dimensions,Platform} from "react-native";
  ...
  getData(pathRoot){
    ...
    .then(snapshot => {
      if(snapshot == null)
      {
        ...
        data.sort(fun.FunctionDirectory.dynamicSortMultiple("eventStart"))
        _this.setState({
  ...
  renderItem(data){
    ...
    return (
      <TouchableOpacity  onPress={()=>{this.openDetails(item)}}>
        <Smartrow isListing={true} item={item} display={this.state.pr} haveThumbnails={this.state.pr.listingSetup.haveThumbnails} thumbPrefix={this.state.pr.listingSetup.thubnails_prefix} isSpecial={item.isSpecial}>
        </Smartrow>
      </TouchableOpacity>
    )
  }
      ...
      <LinearGradient colors={bgGradient} style={[css.layout.containerBackground,{flex:1}]}>
        ...
        <ConditionalDisplay condition={this.state.pr && this.state.pr.listingSetup && this.state.pr.listingSetup.hasFeatures && this.state.lessThen4Items}> 
          <Carousel
                ...
                layout={'default'}
                containerCustomStyle={{marginTop: 15,marginBottom:10,
                  overflow: 'visible'}}
        ...
        <FlatList
         style={{marginTop: this.state.pr&&this.state.pr.listingSetup&&this.state.pr.listingSetup.hasFeatures?10:0}}

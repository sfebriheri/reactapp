...
export default class ListOfUsers extends Component {
  ...
  */
  getUsers(){
    var _this=this;
    var data=[];
    firebase.database().ref('/users/'+this.state.userId+"/"+this.state.listOf+"/").once('value', function(snapshot) {
 ...
 renderItem(data){
  ...
  openProfile(item){
    var profileScreen=Config.profileScreensInSubMenu?"Sub"+Config.profileScreen:Config.profileScreen;
